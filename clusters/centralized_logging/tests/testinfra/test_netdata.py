"""Live checks for the Netdata real-time agent (:19999) on all three VMs.

Skip-not-fail when `enable_netdata` is off. The kickstart installer is slow on first
boot, so the port/HTTP checks poll with a deadline. The docker VM runs the cluster's
Prometheus, so we also assert the `logging-netdata` scrape job reports up==1.
"""
...
NETDATA_ROLES = ["central", "k0s", "docker"]
...
def test_netdata_target_up_in_prometheus(docker, require_netdata):
    """The docker VM's Prometheus must report up==1 for the logging-netdata targets."""
    deadline = time.time() + BOOT_TIMEOUT
    last = []
    while time.time() < deadline:
        res = docker.run(
            "curl -fsS 'http://localhost:9090/api/v1/query?query=up%7Bjob%3D%22logging-netdata%22%7D'"
        )
        if res.rc == 0 and res.stdout.strip():
            ...
            if last and any(r["value"][1] == "1" for r in last):
                return
        time.sleep(10)
    pytest.fail(f"no up==1 logging-netdata target after {BOOT_TIMEOUT}s (last: {last})")
