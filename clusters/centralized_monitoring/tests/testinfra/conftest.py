...
`enabled_exporters` comes straight from the tofu output so the live suite asserts
only the exporters/integrations that are actually turned on (a disabled feature is
skipped, not failed).
"""
...
def enabled_exporters(tofu_output):
    """Sorted list of active enable_* flags — drives parametrized skips."""
