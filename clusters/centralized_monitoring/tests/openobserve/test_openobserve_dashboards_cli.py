"""Hermetic behavior tests for the openobserve_cli `dashboards` sub-typer + `check
--require-dashboards`, plus a validity sweep over the shipped dashboard JSON.
...
# tests/openobserve/ -> centralized_monitoring/openobserve/dashboards/
SHIPPED_DIR = Path(__file__).resolve().parents[2] / "openobserve" / "dashboards"
EXPECTED_TITLES = {
    "Log Overview",
    "Error Triage",
    "Per-Host / Per-Stream",
...
def _write_dash(path: Path, title: str):
    ...
    path.write_text(
        json.dumps({"version": 5, "title": title, "tabs": [{"panels": []}]})
    )
...
def _base_check_server(httpserver, *, dashboards=None):
    ...
    httpserver.expect_request(
        "/api/v2/default/folders/dashboards", method="GET"
    ).respond_with_json({"list": []})
    httpserver.expect_request(
        "/api/default/dashboards", method="GET"
    ).respond_with_json({"dashboards": dashboards or []})
    return httpserver.url_for("")


def _panels(dash: dict) -> list:
...
def test_dashboards_list_reads_folders_and_dashboards(httpserver):
    httpserver.expect_request(
        "/api/v2/default/folders/dashboards", method="GET"
    ).respond_with_json({"list": [{"folderId": "Logs", "name": "Logs"}]})
    httpserver.expect_request(
        "/api/default/dashboards", method="GET"
    ).respond_with_json(
...
def test_delete_sends_delete_request(httpserver):
    httpserver.expect_request(
        "/api/default/dashboards/xyz", method="DELETE"
...
def test_check_require_dashboards_passes_when_present(httpserver, tmp_path):
    ...
    assert any(
        c["name"] == "dashboards present" and c["status"] == "pass" for c in checks
    )
...
def test_check_require_dashboards_fails_when_missing(httpserver, tmp_path):
    ...
    checks = json.loads(r.output)["checks"]
    assert any(
        c["name"] == "dashboards present" and c["status"] == "fail" for c in checks
    )
