"""Hermetic unit tests for _obs_common (TDD).
...
def test_resolve_target_via_tofu_builds_url_and_flags():
    ...
    t = oc.resolve_target(port=5080, cluster="centralized_monitoring", runner=lambda chdir: data)
    assert t.base_url == "http://10.9.8.7:5080"
    assert t.ip == "10.9.8.7"
    assert t.enabled_flags == {"enable_openobserve"}


def test_resolve_target_passes_cluster_chdir_to_runner():
    seen = {}
    ...
    oc.resolve_target(port=9090, cluster="centralized_logging", runner=runner)
...
def test_resolve_credentials_explicit_wins():
    assert oc.resolve_credentials(
        "u", "p", default_user="admin", default_password="admin"
    ) == ("u", "p")
