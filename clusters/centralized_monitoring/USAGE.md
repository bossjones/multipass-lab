...
| verify APIs | `just verify-api centralized_monitoring` | host-side Grafana/Prometheus/OpenObserve `check` (OpenObserve asserts live ingestion via `--require-metrics --require-logs`) |
| generate load | `just locust centralized_monitoring` | host-run Locust web UI (`localhost:8089`) drives traffic into the ingest/query surfaces; `just locust-check` for a CI smoke run |
...
- **`cloud-init/prometheus/prometheus.yml.tftpl`** — one `%{ if enable_x ~}` scrape job per
  feature; client jobs target `${k0s_ip}:<port>`. When `enable_openobserve` is on it also adds a
  **`remote_write`** block shipping every scraped series into OpenObserve
  (`/api/default/prometheus/api/v1/write`, basic auth).
- **`cloud-init/otel/collector-config.yaml.tftpl`** — the server OTel Collector: OTLP receivers
  (`:4317/:4318`) plus **`filelog`** receivers tailing Docker container logs
  (`/var/lib/docker/containers/*/*.log`) and host `/var/log/syslog`, exported to OpenObserve with
  per-stream `stream-name` headers (`container_logs`, `host_logs`, `otlp_logs`).
- **`cloud-init/otel/k0s-collector-config.yaml.tftpl`** — an `otelcol-contrib` agent on the k0s VM
  shipping node syslog + Kubernetes pod logs to the server's OpenObserve (streams `k0s_host` /
  `k0s_pods`). Its server endpoint is injected **post-apply** via `terraform_data.k0s_log_shipper`
  (`multipass transfer` + `systemctl restart`), since the k0s VM boots before the server exists.
...
| tier | flags |
|------|-------|
| MVP (on) | `enable_otel` `enable_openobserve` `enable_k0s_log_shipping` `enable_blackbox` `enable_node_exporter` `enable_cadvisor` `enable_process_exporter` `enable_netdata` |
