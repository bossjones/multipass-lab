...
| `just check <cluster>` fails on a `*_off_by_default` assertion | A leftover `clusters/<cluster>/.cross-cluster.auto.tfvars.json` from a previous `just up-connected` run is setting `internal_ca_cert` (or `dns_server`/`log_shipping_target`/etc.) to a non-empty value. OpenTofu auto-loads any `*.auto.tfvars.json` in the module directory — including during `tofu test` — so the "off by default" run's assumption that the var is unset is violated. | `mv clusters/<cluster>/.cross-cluster.auto.tfvars.json /path/to/scratchpad/` (don't `rm` it under the sandbox's `pre_tool_use` guard — see `CLAUDE.md`) before running `just check`, or just re-run `just check` right after a fresh `just up <cluster>` (no cross-cluster file written) rather than after `up-connected`. |
