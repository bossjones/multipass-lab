...
description: Reference knowledge for triaging Multipass/cloud-init provisioning logs — the strong failure signatures worth flagging, the canonical repo root-cause mappings (otelcol, cloud-init oneshots, DNS races, netbox async launch), and what counts as benign first-boot noise. Consult this whenever reading journalctl/cloud-init output from a cluster VM, deciding whether a log line is signal or noise, or naming the root cause behind a `tools/system_debug.py` signature hit. Preloaded into the log-researcher subagent so every parallel investigator applies identical triage.
